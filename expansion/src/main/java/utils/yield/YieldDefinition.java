...
import static utils.yield.YieldCompleted.completed;
import static utils.yield.FlowControl.youMayProceed;
import static utils.yield.IfAbsent.ifAbsent;
import static utils.yield.YieldMessage.message;
...
public class YieldDefinition<T> implements Iterable<T>, ClosableIterator<T> {
    private final SynchronousQueue<YieldMessage<T>> dataChannel = new SynchronousQueue<>();
    ...
    public boolean hasNext() {
        calculateNextValue();
        YieldMessage<T> message = unchecked(() -> dataChannel.take());
        if (message instanceof YieldCompleted) return false;
