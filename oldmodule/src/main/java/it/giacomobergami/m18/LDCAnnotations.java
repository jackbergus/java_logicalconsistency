...
//            TuplesDao dao = new TuplesDao(Database.open(StaticDatabaseClass.engine, databaseId, StaticDatabaseClass.username, StaticDatabaseClass.password).get().jooq().configuration());
            /*for (Tuples x :
                    Utils.fetchTuplesByExpansionId(Database.open(StaticDatabaseClass.engine, databaseId, StaticDatabaseClass.username, StaticDatabaseClass.password).get().jooq(), map.keySet().toArray(new String[map.size()]))) {
                ...
                tup.negated = x.getNegated();
                tup.hedged = x.getHedged();
                tup.score = x.getScore();

                for (int i1 = 0, arrayAggLength = arrayAgg.length; i1 < arrayAggLength; i1++) {
                    Object arg = arrayAgg[i1];
                    try {
                        // While deserializing the object, it calls the AgileField constructor from the json object
            ...
            for (Tuples2 x :
                    Utils.fetchTuples2ByExpansionId(Database.open(StaticDatabaseClass.engine, databaseId, StaticDatabaseClass.username, StaticDatabaseClass.password).get().jooq(), map.keySet().toArray(new String[map.size()]))) {
                try {
                    HashMultimap<String, AgileField> mm = HashMultimap.create();
                    HashSet<String> constituents = new HashSet<>(Arrays.asList(x.getConstituent()));
                    {
                        Map<String, List<AgileField>> mappa = reader2.readValue(((PGobject) x.getJsonObjectAgg()).getValue());
                        mappa.forEach(mm::putAll);
                    }
                    AgileRecord ar = new AgileRecord(x.getNisttype(), mm, constituents, x.getNegated(), x.getHedged(), x.getScoreevent());
                    String value = map.get(x.getMid());
                    if (value.equals("contradicts")) {
                        contradictory.put(x.getNisttype(),  ar);
                    } else if (value.equals("fully-relevant")) {
                        fullyRelevant.put(x.getNisttype(),  ar);
                    }else if (value.equals("partially-relevant")) {
                        partRelevant.put(x.getNisttype(),  ar);
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            print(fullyRelevant, partRelevant, contradictory);
        }
    }
