...
public class LoadingHypothesesFromTSV {
    ...
    public static void main(String parg[]) throws IOException {
        ...
        String path = "/home/giacomo/Scrivania/evaluation/sama_restrict";
            ...
            for (File f : Objects.requireNonNull(new File(path).listFiles())) {
                ...
                while (s.hasNextLine()) {
                    ...
                    if (testing_function.apply(line[2], currentRange)) {
                        ...
                        Collections.addAll(erArguments, line[1].split("\\|"));
                        Set<String> erWithArguments = new HashSet<>(erArguments);
                        Collections.addAll(erWithArguments, line[0].split("\\|"));
                        ...
                        if (cumulative.isEmpty()) {
                            ...
                            for (String eventIds : erArguments) {
                                lines.add(hypoFile+","+hypothesisIdByCounting+",NA,"+eventIds+",NA,"+currentRange);
                            }
                        } else {
                            ////
                            ...
                            for (String associatedHypothesis : cp.getKey()) {
                                for (String eventIds : erArguments) {
                                    lines.add(hypoFile+","+hypothesisIdByCounting+","+associatedHypothesis+","+eventIds+","+classification.get(new Pair<>(associatedHypothesis, eventIds))+","+currentRange);
                                }
        ...
        PrintWriter pw = new PrintWriter(new FileOutputStream(new File(path, "classification_full.csv")));
        pw.println("query_id,subgraph_id,matched_hypothesis,er,annotated_as,lambda_score");
        for (String x : lines)
            pw.println(x);
        pw.close();
