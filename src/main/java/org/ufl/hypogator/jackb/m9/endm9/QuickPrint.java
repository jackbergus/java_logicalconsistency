...
public class QuickPrint extends StaticDatabaseClass {
...
// substring in str
    ...
    public static void main(String args[]) throws IOException {
        loadProperties();
        Database opt = Database.openOrCreate(engine, "gaia3", username, password).get();
        ...
        //File f = new File("/home/giacomo/Scrivania/Inconsistenza/out/graph_output_BBN_F001_Q004_len_2_pathlen_test_K_10-10-18-2018_15:04:49.json");
        for (File f : new File("/home/giacomo/Dropbox (UFL)/AIDA baseline/Shared V6/v6/visualization_output/10-18-2018").listFiles()) {
            if (!f.getName().contains("GAIA_3")) continue;
            ...
            for (int i = 0; i < subgraphs.length; i++) {
                ...
                if (!Arrays.stream(s.scorers).filter(x -> x.name.endsWith("_li")).anyMatch(x->x.value > 0)) continue;
                    ...
                    for (int k = 0, subgraph_plus_neighborsLength = subgraph_plus_neighbors.length; k < subgraph_plus_neighborsLength; k++) {
                        ...
                        if (neigh.node_text_1.trim().isEmpty()) {
                            ...
                            if (! tp.contains(neigh.node_id_1)) {
                                List<Tuples> tuple = dao.fetchByMid(neigh.node_id_1);
